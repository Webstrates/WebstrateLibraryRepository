...
class UberTracker {
    constructor(options) {
        ...
        window.addEventListener("message", (event)=>{
            try {
                if (event.data?.ubertrackercommand==="scroll"){
                    // STUB: Assume document element
                    let element = window;
                    element.scrollTo(event.data.x, event.data.y);
                }
            } catch (ex){
                console.log("UbertTrack message exception ignored", ex);
            }
        })
